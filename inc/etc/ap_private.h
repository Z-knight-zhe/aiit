...
namespace AESL_std {
    ...
    DataType INLINE minAESL(DataType a, DataType b) {
        //    if (a >= b) return b;
        //    else return a;
        return (a>=b) ? b : a;
    }
    ...
    template <class DataType>
    DataType INLINE maxAESL(DataType a, DataType b) {
        //    if (a >= b) return a;
        //    else return b;
        return (a>=b) ? a : b;
    }
}
#else//#elif defined LINUX   // linux
namespace AESL_std {
    template <class DataType>
...
static INLINE bool add(uint64_t *dest, const uint64_t *x, const uint64_t *y, 
    ...
    bool carry = false;
    uint32_t len = AESL_std::minAESL(xlen, ylen);
    uint32_t i;
    for (i = 0; i< len && i < destlen; ++i) {
        uint64_t limit = AESL_std::minAESL(x[i],y[i]); // must come first in case dest == x
    ...
    if (xlen > ylen) {
        ...
        for (i=ylen; i< xlen && i < destlen; i++) {
            uint64_t limit = AESL_std::minAESL(x[i], yext);
        ...
        for (i=xlen; i< ylen && i < destlen; i++) {
            uint64_t limit = AESL_std::minAESL(xext, y[i]);
...
static INLINE void KnuthDiv(uint32_t *u, uint32_t *v, uint32_t *q, uint32_t* r, 
    ...
    do {
        ...
        if (isNeg) {
            ...
            for (uint32_t i = 0; i < n; i++) {
#if defined WIN32 || defined WIN64
                uint32_t limit = AESL_std::minAESL(u[j+i],v[i]);
#else//#elif defined LINUX   // linux
...
//just for clang compiler
    ...
    uint32_t countLeadingZeros() const {
        ...
        if (Count) 
            Count-=excessBits;
        return AESL_std::minAESL(Count, (uint32_t)_AP_W);
    }
#else//#elif defined LINUX   // linux
    uint32_t countLeadingZeros() const {
        int remainder = BitWidth % 64;
...
private:
    ...
    ap_private(uint32_t numWords, const uint64_t bigVal[]) {
        ...
        assert(bigVal && "Null pointer detected!");
        {
            ...
            uint32_t words = AESL_std::minAESL<uint32_t>(numWords, _AP_N);
...
#define OP_BIN_LOGIC_ASSIGN_AP(Sym)              \
    ...
    INLINE ap_private& operator Sym(const ap_private<_AP_W1, _AP_S1>& RHS) {    \
        ...
        uint32_t numWords = AESL_std::minAESL((int)_AP_N, _AP_N1);                        \
        uint32_t i;                                                              \
...
#define OP_BIN_LOGIC_AP(Sym)                                                \
    ...
    operator Sym (const ap_private<_AP_W1, _AP_S1>& RHS) const {    \
        ...
        uint32_t i;                                                         \
        const int _AP_N1 = ap_private<_AP_W1, _AP_S1>::_AP_N;               \
        uint32_t min_N = AESL_std::minAESL((int)_AP_N, _AP_N1);                           \
        uint32_t max_N = AESL_std::maxAESL((int)_AP_N, _AP_N1);                           \
        for (i = 0; i < min_N; ++i)                                         \
...
#undef OP_ASSIGN_AP
...
#if defined WIN32 || defined WIN64
        uint32_t topWord = whichWord(AESL_std::maxAESL(n1,n2)-1);
#else
    ...
    INLINE uint32_t countTrailingZeros() const {
        ...
        return AESL_std::minAESL(Count, BitWidth);
    }
#else//#elif defined LINUX   // linux
    INLINE uint32_t countTrailingZeros() const {
        uint32_t Count = 0;
